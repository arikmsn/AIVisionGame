 ...
 *
 * NOTE: trailing spaces (e.g. 'fed ', 'sol ') were previously used to
 * force word-boundary matching via includes(). classifyMarketDomain now
 * uses \b regex word-boundaries, so they are stripped automatically.
 */
 ...
 *
 * Uses \b word-boundary matching so that e.g. "senators" does not trigger
 * the "nato" geopolitics keyword, and "alfred" does not trigger "fed".
 */
export function classifyMarketDomain(title: string, category: string | null | undefined): Domain {
  ...
  for (const [domain, kws] of Object.entries(DOMAIN_KEYWORDS) as [Exclude<Domain, 'other'>, string[]][]) {
    if (kws.some(k => new RegExp(`\\b${reEscape(k.trim())}\\b`).test(text))) return domain;
  }
  return 'other';
}
